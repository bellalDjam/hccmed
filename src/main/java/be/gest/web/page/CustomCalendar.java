...
import be.gest.web.CalendarTestEvent;
import be.gest.web.MainUI;

@UIScope
@SpringComponent
public class CustomCalendar extends AbstractPage{

	/**
	 * 
	 */
	private static final long serialVersionUID = 4479176324264830592L;
	...
	public String getPageTitle() {
	 ...
	 private static final String DEFAULT_ITEMID = "DEFAULT";
	    ...
	    private Locale defaultLocale = Locale.FRANCE;
...
//	    }
	    ...
	    public void initContent() {
	        ...
	        updateCalendarLocale(Locale.getDefault());
	    }
	    ...
	    private void addInitialEvents() {
	        ...
	        event=geSumFacturesEvent(start);
	        //event.setAllDay(true);
	        event.setStyleName("color4");
	        event.setDescription("10000");
...
//	        dataSource.addEvent(event);
	    ...
	    private void initLayoutContent() {
	        ...
	        controlPanel.addComponent(addNewMission);
	        ...
	        controlPanel.setComponentAlignment(hideWeekendsButton,
	                Alignment.MIDDLE_LEFT);
	        controlPanel.setComponentAlignment(addNewMission, Alignment.MIDDLE_LEFT);
	    ...
	    public void initAddNewMissionButton() {
	        addNewMission = new Button("Ajouter mission");
	        addNewMission.addClickListener(new Button.ClickListener() {
	    ...
	    private void showEventPopup(CalendarEvent event, boolean newEvent) {
	        ...
	        if (!MainUI.getInstance().getWindows().contains(scheduleEventPopup)) {
	        	MainUI.getInstance().addWindow(scheduleEventPopup);
	        }
	    }
	    ...
	    private void deleteCalendarEvent() {
	        ...
	        MainUI.getInstance().removeWindow(scheduleEventPopup);
	    }
	    ...
	    private CalendarTestEvent geSumFacturesEvent(Date currentDate) {
	        CalendarTestEvent event = new CalendarTestEvent();
//	        event.setCaption(echeanceToPay(currentDate));
	        event.setCaption("1000000");
	        event.setStart(currentDate);
	        event.setEnd(currentDate);
//	        FactureDAO	facturDao = new FactureDAOImp();
//	        System.out.println(fournisseurDAO.findAll());

	        return event;
	    } 
	    public String  echeanceToPay(Date date){
//	    	FactureDAO facturDao = new  FactureDAO();
	    	if(facturDao.findAllImpayedByDate(date)!=null && facturDao.echeanceByDate(date) !=BigDecimal.ZERO){
	    		return facturDao.echeanceByDate(date).toString();
